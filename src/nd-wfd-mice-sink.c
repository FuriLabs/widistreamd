...
#include "widi-stream-enum-types.h"
...
nd_wfd_mice_sink_sink_start_stream (NdSink *sink)
{
  ...
  if (missing_video)
    {
      for (guint i = 0; missing_video[i] != NULL; i++)
        g_debug ("NdWFDMiceSink: Missing video codec: %s", missing_video[i]);
    }

  if (missing_audio)
    {
      for (guint i = 0; missing_audio[i] != NULL; i++)
        g_debug ("NdWFDMiceSink: Missing audio codec: %s", missing_audio[i]);
    }
