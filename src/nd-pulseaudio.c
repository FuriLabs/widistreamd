                                         ...
                                         gpointer            user_data)
{
  ...
  g_autofree gchar *app_name = g_strdup_printf ("WiDiStream PulseAudio Client for %s", self->name);
  g_autofree gchar *app_id = g_strdup_printf ("io.furios.WiDiStream.PulseAudio_%.8s", self->uuid);
...
nd_pulseaudio_get_source (NdPulseaudio *self)
{
  ...
  g_autofree gchar *client_name = g_strdup_printf ("WidiStream Audio Grabber for %s", self->name);
  g_object_set (src,
                "device", device_name,
